  ...
  stages {
    stage('Build') {
      steps {
        ...
        container('helm') {
          sh """
            helm upgrade -i --namespace ma-services services services --set image.repo=europe-west2-docker.pkg.dev/yamp-playground/yamp-registry/facts-service --set image.tag=${BUILD_NUMBER}
          """
          }
        }
    }
